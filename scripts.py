...
def start():
    run_process('poetry run py -m telegram_param_store.main')
...
def compile_proto():
    RELATIVE_PROTO_FILE_PATH = 'telegram_param_store\shared_utils\protos.proto'
    command = f"poetry run py -m grpc_tools.protoc --proto_path=. {RELATIVE_PROTO_FILE_PATH} --python_out=. --grpc_python_out=."
    run_process(command)
...
def run_process(commands: str):
    """run a process
    used to execute commands such as - python -m pip install"""
    try:
...
def run_process__parallel(commands: List[str]):
    try:
